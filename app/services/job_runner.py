...
def _process_job_sync(job_id: str) -> None:
    ...
    try:
        ...
        def _build_query_payload(query_item) -> Dict:
            if isinstance(query_item, str):
                raw_value = query_item
                manufacturer_value = extract_query_manufacturer(str(raw_value or ""))
                return {
                    "name": query_item,
                    "qty": None,
                    "dosage": None,
            ...
            q_name_value = (query_item.get("name") or "").strip()
            raw_value = query_item.get("raw") or query_item.get("row") or q_name_value
            manufacturer_value = (query_item.get("manufacturer") or "").strip()
            if not manufacturer_value:
                manufacturer_value = extract_query_manufacturer(str(raw_value or ""))
            return {
                "name": q_name_value,
                "qty": query_item.get("qty", None),
                "dosage": query_item.get("dosage", None),
                "qty_is_sum": bool(query_item.get("qty_is_sum", False)),
                "raw": raw_value,
                "barcode": (query_item.get("barcode") or "").strip(),
                "product_code": (query_item.get("product_code") or "").strip(),
                "manufacturer": manufacturer_value,
            }
        ...
        def _run_pharmacy_worker(pharmacy_code: str) -> None:
            ...
            try:
                ...
                for q_payload in normalized_queries:
                    if cancel_requested():
                        return
                    ...
                    q_qty = q_payload["qty"]
                    q_dosage = q_payload["dosage"]
                    q_sum = q_payload["qty_is_sum"]
                    raw = q_payload["raw"]
                    q_barcode = q_payload["barcode"]
                    q_product_code = q_payload["product_code"]
                    q_manufacturer = q_payload["manufacturer"]
                    ...
                    try:
                        if pharmacy_code == "apteka_ru":
                            ...
                            if local_farmacia24_parser is None:
                            ...
                            outcome_data = local_farmacia24_parser.parse_one(
                                ParseQuery(
                                    name=q_name,
                                    raw=str(raw or ""),
                                    qty=q_qty,
                ...
                if local_driver is not None:
                    try:
                        ...
                        pass
                if local_farmacia24_parser is not None:
                    try:
                        local_farmacia24_parser.close()
                    except Exception:
                        pass
